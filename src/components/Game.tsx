...
export default function Game({ lineId,stations,color,sound=true,durationSeconds,showAllStations=true,onExit }:{ lineId?:string; stations:string[]; color:string; sound?:boolean; durationSeconds?:number; showAllStations?:boolean; onExit:()=>void }) {
  ...
  const line2Width=(name:string)=>Math.max(330,Math.min(680,170+Math.max(name.length*50,(LINE_2_BY_NAME.get(name)?.english.length??0)*13)))
  ...
  return <section className="game" style={gameStyle}>
    ...
    {line2Tracking?<Line2TypingField target={target} number={LINE_2_BY_NAME.get(target)?.number??''} value={value} inputRef={input} onChange={changeInput} onKeyDown={submit} />:<input ref={input} value={value} onChange={changeInput} onKeyDown={submit} placeholder="역명을 입력하고 Enter" aria-label="역명 입력" autoComplete="off" />}
  </section>
}
