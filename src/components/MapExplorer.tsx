    ...
    {city==='seoul' ? <>
      <div className="map-frame" data-testid="seoul-map" data-active-line={activeLine??undefined}>
        <div className="map-stage">
          <img src={asset('seoul-metro-map-20250929.jpg')} srcSet={`${asset('seoul-metro-map-20250929.jpg')} 5102w, ${asset('seoul-metro-map-20250929@2x.webp')} 10205w`} sizes="(max-width: 900px) 850px, 100vw" alt="서울 수도권 지하철 노선도" />
          <span className="map-dimmer" aria-hidden="true" />
          <svg className="map-hitareas" viewBox="0 0 2551.18 2551.18" aria-label="지원 노선 선택">
            {hitPaths.map(line=><g key={line.id} role="button" tabIndex={0} aria-label={`지도에서 ${line.name} 선택`} data-active={activeLine===line.id} style={{'--line':line.color} as React.CSSProperties} onPointerEnter={()=>beginHover(line.id)} onPointerLeave={endHover} onFocus={()=>focusLine(line.id)} onBlur={endHover} onClick={()=>onSelect(line.id)} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();onSelect(line.id)}}}><use className="line-halo" href={`${asset('seoul-supported-lines.svg')}#${line.id}`} /><use className="line-highlight" href={`${asset('seoul-supported-lines.svg')}#${line.id}`} /><use className="line-hit" href={`${asset('seoul-supported-lines.svg')}#${line.id}`} /></g>)}
          </svg>
        </div>
      </div>
