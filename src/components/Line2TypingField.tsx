...
export default function Line2TypingField({target,number,value,errorAttempt,inputRef,onChange,onKeyDown}:{target:string;number:string;value:string;errorAttempt:number;inputRef:RefObject<HTMLInputElement|null>;onChange:ChangeEventHandler<HTMLInputElement>;onKeyDown:KeyboardEventHandler<HTMLInputElement>}) {
  const targetCharacters=Array.from(target),typed=Array.from(value.normalize('NFC'))
  return <div className="line2-typing-field">
    <div className="line2-typing-visual" data-error-attempt={errorAttempt} key={errorAttempt}>
