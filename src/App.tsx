...
import { getQuickRoutePairs, getRoute, getStations, type QuickRoute } from './game/routes'
...
export default function App() {
  const [mode,setMode]=useState<'route'|'transfer'|null>(null)
  ...
  const selectLine=(id:string)=>{
    ...
    setLineId(id);setMode('transfer');setServiceId(selectedLine?.services?.[0]?.id);setFrom('');setTo('');setToward('');setRouteOverride(null);setSetupError('')
  }
  ...
  if (playing && line) {
    ...
    const route=routeOverride??getRoute(line.id,from,to,line.loop?direction:'forward',serviceId).stationIds
    return <main className="shell"><ErrorBoundary onReset={()=>{setPlaying(false);setRouteOverride(null)}}><Game lineId={line.id} stations={route} color={line.color} sound={sound} onExit={()=>{setPlaying(false);setRouteOverride(null)}} /></ErrorBoundary></main>
  }
  return (
    <main className="shell">
      ...
      {line ? <section className="hero setup">
        ...
        <div className="setup-mode"><button className={mode==='transfer'?'active':''} onClick={()=>{setMode('transfer');setRouteOverride(null);setToward('')}}>환승 여행</button><button className={mode==='route'?'active':''} onClick={()=>{setMode('route');setRouteOverride(null)}}>노선 운행</button></div>
        ...
        <button className="primary" style={{background:line.color}} disabled={(mode==='route'&&(!from||!to||from===to))||(mode==='transfer'&&(!from||!toward))} onClick={()=>{if(mode==='transfer'){setPlaying(true)}else tryStartRoute()}}>운행 시작 →</button>
